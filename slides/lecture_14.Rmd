...
.large[
Suppose we include passenger class in the model instead of Fare:

$\log \left( \dfrac{p_i}{1 - p_i} \right) = \beta_0 + \beta_1 Sex_i + \beta_2 FirstClass_i + \beta_3 SecondClass_i \\ \hspace{3cm} + \beta_4 Age_i + \beta_5 Age_i \cdot Sex_i$
]

.large[
.question[
How would I test the hypothesis that second class passengers have the same chance of survival as third class passengers (after accounting for Sex and Age)?
]
]
...
.large[
.question[
How would I test the hypothesis that second class passengers have the same chance of survival as *first* class passengers (after accounting for Sex and Age)?
]
]
